...
def _worker_token_path(worker_id: str, explicit_path: str | None) -> Path:
    ...
    return Path(configured).expanduser() if configured else _default_worker_token_path(worker_id)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="AgentHub macOS worker")
