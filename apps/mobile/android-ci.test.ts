...
describe('Android APK GitHub Actions workflow', () => {
  ...
  it('uses the shared AgentHub brand icon for Android launchers', () => {
    ...
    expect(foreground).toContain('#6CC8FF');
    expect(foreground).toContain('#58B6FF');
    expect(foreground).toContain('M44.53,13.61');
    expect(notificationIcon).toContain('M9.91,3.02');
    expect(webFavicon).toContain('<title>AgentHub</title>');
    expect(webFavicon).toContain('#6CC8FF');
    expect(webFavicon).toContain('#58B6FF');
