export type NativeTabKey = 'sessions' | 'tasks' | 'files' | 'workers' | 'me';

export interface NativeTabDefinition {
  key: NativeTabKey;
  label: string;
  icon: string;
}

export const nativeTabs: readonly NativeTabDefinition[] = [
  { key: 'sessions', label: '会话', icon: 'chatbubble-ellipses-outline' },
  { key: 'tasks', label: '任务', icon: 'play-circle-outline' },
  { key: 'files', label: '文件', icon: 'folder-outline' },
  { key: 'workers', label: '节点', icon: 'hardware-chip-outline' },
  { key: 'me', label: '我的', icon: 'person-circle-outline' },
];
