...
import { DEFAULT_ELECTRON_DOWNLOAD_CHUNK_SIZE, electronArchiveName, electronArchiveUrl } from './package-win-config.mjs';

export async function prewarmElectronDistCache({
  cacheDir,
  env = process.env,
  ...
  if (actualSize !== expectedSize) {
    console.warn(
      `Chunked Electron download size mismatch for ${archiveName}: expected ${expectedSize}, got ${actualSize}. Falling back to a single-file download.`,
    );
    rmSync(tempPath, { force: true });
    downloadWholeFile(archiveUrl, archivePath, env, expectedSize);
...
function downloadWholeFile(url, archivePath, env, expectedSize) {
  const tempPath = `${archivePath}.full.tmp`;
  rmSync(tempPath, { force: true });
  runCurl(
    ['-L', '--fail', '--retry', '5', '--retry-all-errors', '--output', tempPath, url],
    env,
    { stdio: 'inherit' },
  );
  const actualSize = statSync(tempPath).size;
  if (actualSize !== expectedSize) {
    rmSync(tempPath, { force: true });
    throw new Error(`Electron archive size mismatch after full download: expected ${expectedSize}, got ${actualSize}`);
  }
  rmSync(archivePath, { force: true });
  renameSync(tempPath, archivePath);
}
